...
class LightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        self.std_space = torch.empty((self.n_users, self.n_items,5)).to(self.device)
...
#         self.tmp = 500
    ...
    def forward(self, cur_epoch, batch=None ,generator=None):
        ...
        if self.ns == 'rns':  # n_negs = 1
            ...
            for k in range(self.K):
                neg_gcn_embs.append(self.augmented_negative_sampling(user_gcn_emb, item_gcn_emb,
                                                                   user,
    ...
    def augmented_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates,pos_item ,generator):
        ...
        magnitude = torch.nn.functional.normalize(random_noise, p=2, dim=-1) * margin
        ...
        ans_loss  = 0
        ans_loss += torch.mean(torch.log(1 + torch.exp(easy_scores - hard_scores).sum(dim=1)))
        
        self.ans_loss = ans_loss 
        self.dis_loss = distance + orth

        """dynamic negative sampling"""
        scores = (s_e.unsqueeze(dim=1) * n_e_).sum(dim=-1)  # [batch_size, n_negs]
        infor_gain =  syth_scores - norm_scores
        
        
        
        indices = torch.max(scores + self.eps * infor_gain, dim=1)[1].detach()  # [batch_size]
    ...
    def create_bpr_loss(self, cur_epoch, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        ...
        if self.ns == 'ans':
            ...
            mf_loss += self.gamma * self.ans_loss
            mf_loss += self.gamma * self.dis_loss
